...
export async function seedPsychologistsToFirebase(): Promise<void> {
    ...
    const psychologistsForUpload = psychologists.reduce<Record<string, Omit<Psychologist, "id">>>((accumulator, psychologist) => {
...
export async function fetchPsychologistsFromFirebase(): Promise<Psychologist[]> {
    const databaseUrl = ensureDatabaseUrl();
    const response = await fetch(`${databaseUrl}/psychologist.json`);

    if (!response.ok) {
        throw new Error("Failed to fetch psychologists from Firebase");
    }
